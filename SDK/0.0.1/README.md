...
`tools/wuw_stream_wav` is a runnable example. `tools/wuw_eval_manifest` supports deterministic modulo shards and records the sliding-window index for every accepted event. `scripts/submit_expts8_eval.sh` evaluates 5.12-second windows with a 2.56-second stride, waits for and validates the sbatch array, and writes `summary.json` plus `FULL_CONDITION_COMPARISON.md`. The report uses the single stage-2 threshold in `sdk_defaults.json`; for negative data, FA rate is accepted windows divided by evaluated windows, while FA/hour uses debounced events over source-audio duration.
