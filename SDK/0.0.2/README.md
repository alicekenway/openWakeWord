...
One engine may create multiple streams for different microphone zones. The
streams may run concurrently, but one stream must be called by only one thread
at a time. `Reset()` is the lifecycle boundary that permits ownership to move
to a newly created worker thread; it must not race with another stream call.
