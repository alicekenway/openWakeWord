...
int main(int argc,char**argv){
  try{
    ...
    while(std::getline(in,line)){
      ...
      try{
        tool::WavReader wav(path);std::vector<WuwSdkEvent>events;std::vector<uint64_t>event_windows;std::vector<double>window_rtfs;uint64_t processing_us=0,window_id=0;
        auto process=[&](const int16_t*data,size_t samples,int64_t first){const uint64_t current_window=window_id++;stream.Reset();stream.BeginSegment((index<<20)+current_window,first);for(size_t p=0;p<samples;p+=1600)stream.AcceptPcm16(data+p,std::min<size_t>(1600,samples-p));stream.EndSegment();auto found=stream.ReadEvents(64);events.insert(events.end(),found.begin(),found.end());event_windows.insert(event_windows.end(),found.size(),current_window);const auto stats=stream.Stats();processing_us+=stats.processing_time_us;window_rtfs.push_back(stats.processing_time_us/(1000000.0*samples/16000.0));};
        if(!window_samples){std::vector<int16_t>pcm(1600);size_t n=0;stream.Reset();stream.BeginSegment(index,0);while((n=wav.Read(pcm.data(),pcm.size()))!=0)stream.AcceptPcm16(pcm.data(),n);stream.EndSegment();events=stream.ReadEvents(64);event_windows.assign(events.size(),0);processing_us=stream.Stats().processing_time_us;window_id=1;window_rtfs.push_back(processing_us/(1000000.0*wav.total_samples()/16000.0));}
        ...
        out<<"{\"source_index\":"<<index<<",\"id\":\""<<tool::Escape(id)<<"\",\"path\":\""<<tool::Escape(path)<<"\",\"text\":\""<<tool::Escape(transcript)<<"\",\"expected_keyword_id\":\""<<tool::Escape(expected)<<"\",\"samples\":"<<wav.total_samples()<<",\"duration_seconds\":"<<(wav.total_samples()/16000.0)<<",\"audio_window_count\":"<<window_id<<",\"window_samples\":"<<window_samples<<",\"stride_samples\":"<<stride_samples<<",\"processing_time_us\":"<<processing_us<<",\"rtf\":"<<rtf<<",\"inference_performance\":[";
        for(size_t i=0;i<window_rtfs.size();++i){if(i)out<<',';out<<"{\"audio_window_index\":"<<i<<",\"real_time_factor\":"<<window_rtfs[i]<<'}';}
        out<<"],\"events\":[";
        for(size_t i=0;i<events.size();++i){if(i)out<<',';out<<"{\"audio_window_index\":"<<event_windows[i]<<",\"keyword_id\":\""<<tool::Escape(events[i].keyword_id)<<"\",\"confidence\":"<<events[i].confidence<<",\"start_sample\":"<<events[i].start_sample_index<<",\"end_sample\":"<<events[i].end_sample_index<<'}';}
        out<<"]}\n";
